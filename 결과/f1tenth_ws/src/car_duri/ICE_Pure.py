...
class Obstacle_detect(threading.Thread):
    ...
    def subCallback_od(self, msg_sub):
        ...
        for i in range(1,self.scan_range - 1):
            if (self.scan_origin[i] > 0.3 and self.scan_origin[i]) < 6.0 and (self.scan_origin[i-1]*1.4 < self.scan_origin[i] or self.scan_origin[i-1]*0.6 > self.scan_origin[i]) and (i > 359 and i< 721):
                start_idx_temp = i
                end_idx_temp = i
                max_idx_temp = i
                i = i+1
